...
@router.post("/login")
async def login(body: UserLoginBody, db: AsyncSession = Depends(db_helper.get_db)):
...
async def register(body: UserRegisterBody, db: AsyncSession = Depends(db_helper.get_db)):
    user = await db_get_user_by_username(db, body.username)
    if user:
...
async def update_user(body: UserUpdateBody, db: AsyncSession = Depends(db_helper.get_db)):
    user = await db_get_user_by_username(db, body.username)
    if not user:
